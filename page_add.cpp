...
void Form::on_tableWidget_cetak_add_cellDoubleClicked(int i, int column)
{
    qInfo() << "ini adalah even change tw add ============== " << i <<" === " <<column;
...
void Form::memuatData_twRealisasiadd(QString id_kamp){

   header_twRealisasiadd();
   while(ui->tableWidget_realisasi_add->rowCount()>0)// untuk Hilangkan Tambahan jika button di klik ulang
   {ui->tableWidget_realisasi_add->removeRow(0);}

   double pagu_add=0;
   double tot_cair_add=0;
   double sisa_add=0;

    QSqlQuery query;
    QString cmd = "SELECT pmk_yhk.add.nama_kampung,pmk_yhk.add.nama_distrik,pmk_yhk.add.id_dis, pmk_yhk.add.id_k,pmk_yhk.add.pagu,"
                  "SUM ( pmk_yhk.add_cair.jml_cair_add ) AS total_pencairan, pmk_yhk.add.pagu - SUM ( pmk_yhk.add_cair.jml_cair_add ) AS sisa_dana "
       " FROM "
           " pmk_yhk.add"
           " LEFT JOIN pmk_yhk.add_cair ON pmk_yhk.add.id_k = pmk_yhk.add_cair.id_kam "
        " GROUP BY "
           " pmk_yhk.add.nama_kampung,"
            "pmk_yhk.add.nama_distrik,"
            "pmk_yhk.add.id_dis,"
            "pmk_yhk.add.id_k"
       " ORDER BY "
           "pmk_yhk.add.id_k";

    QString cmd_ = "SELECT pmk_yhk.add.nama_kampung,pmk_yhk.add.nama_distrik,pmk_yhk.add.id_dis, pmk_yhk.add.id_k,pmk_yhk.add.pagu,"
                  "SUM ( pmk_yhk.add_cair.jml_cair_add ) AS total_pencairan, pmk_yhk.add.pagu - SUM ( pmk_yhk.add_cair.jml_cair_add ) AS sisa_dana "
       " FROM "
           " pmk_yhk.add"
           " LEFT JOIN pmk_yhk.add_cair ON pmk_yhk.add.id_k = pmk_yhk.add_cair.id_kam "
                  " WHERE pmk_yhk.add.id_k = :id "
        " GROUP BY "
           " pmk_yhk.add.nama_kampung,"
            "pmk_yhk.add.nama_distrik,"
            "pmk_yhk.add.id_dis,"
    ...
    while (query.next()) {
        ...
        //int i = ui->tableWidget_realisasi_add->rowCount();
        ui->tableWidget_realisasi_add->insertRow(i);
        ...
        double realisasi_add = pagu_d - tot_cair_d;

        QString sisa_dana_s = query.value(6).toString();
        //double sisa_dana_d = sisa_dana_s.toDouble();
        QString sisa_dana_ss = indo.toCurrencyString(realisasi_add, "Rp ");
        ...
        //ui->tableWidget_realisasi_add->setCellWidget(i,0,rb);
        ui->tableWidget_realisasi_add->setItem(i,2,nm_kampung_);
        ui->tableWidget_realisasi_add->setItem(i,1,nm_distrik_);
        ui->tableWidget_realisasi_add->setItem(i,6,id_dis_);
        ui->tableWidget_realisasi_add->setItem(i,0,id_kam_);
        ui->tableWidget_realisasi_add->setItem(i,3,pagu_);
        ui->tableWidget_realisasi_add->setItem(i,5,tot_cair_);
        ui->tableWidget_realisasi_add->setItem(i,4,sisa_dana_);

        i++;

       pagu_add += pagu_d;
       tot_cair_add += tot_cair_d;

    }

    sisa_add = pagu_add - tot_cair_add;

    QString pagu_add_ss = indo.toCurrencyString(pagu_add, "Rp ");
    QString tot_cair_d_ss = indo.toCurrencyString(tot_cair_add, "Rp ");
    QString realisasi_add_ss = indo.toCurrencyString(sisa_add, "Rp ");
...
void Form::header_twRealisasiadd() // Header table widget 6
{
    QStringList headerWidget;
    ui->tableWidget_realisasi_add->setColumnCount(7);
    headerWidget <<"id" <<"Nama Distrik"<<"Nama Kampung" <<"Pagu Anggaran " <<"Sisa Anggaran "<<"Realisasi " <<"id Distrik";
    ui->tableWidget_realisasi_add->setHorizontalHeaderLabels(headerWidget);
    ui->tableWidget_realisasi_add->horizontalHeader()->setStretchLastSection(true);
    ui->tableWidget_realisasi_add->setSelectionBehavior(QAbstractItemView::SelectRows);
    ui->tableWidget_realisasi_add->setColumnWidth(0,8);
    ui->tableWidget_realisasi_add->setColumnHidden(0,true);
    ui->tableWidget_realisasi_add->setColumnWidth(1,140);
    ui->tableWidget_realisasi_add->setColumnWidth(2,150);
    ui->tableWidget_realisasi_add->setColumnHidden(6,true);
    ui->tableWidget_realisasi_add->setColumnWidth(3,120);
    ui->tableWidget_realisasi_add->setColumnWidth(4,120);
    ui->tableWidget_realisasi_add->setColumnWidth(5,120);
   //ui->tableWidget_realisasi_add->setColumnWidth(6,120);
}

void Form::header_cetak_add()
{
    QStringList headerWidget;
...
void Form::muat_data_realisasi_add(QString s_id_kamp)
{
    header_cetak_add();
   ...
   {ui->tableWidget_cetak_add->removeRow(0);}
    QSqlQuery query;
    QString cmd = "SELECT * FROM pmk_yhk.add_cair WHERE id_kam = :id ORDER BY id_real ";
    //if(s_id_kamp==""){cmd ="SELECT * FROM pmk_yhk.t_real ORDER BY id_real ";}
     if(s_id_kamp==""){cmd =" SELECT	* FROM	pmk_yhk.add_cair ORDER BY	id_real ASC, pmk_yhk.add_cair.id_real LIMIT 40 ";}
    query.prepare(cmd);
    query.bindValue(":id",s_id_kamp);
    bool ok = exec(query);
    if(!ok){QMessageBox::information(this,"Error...!!!","Gagal Memuat data realisasi Alokasi Dana Desa..."); return;}
    int i=0;
    while (query.next()) {
...
void Form::btnAdd_add()
{
    ...
    if(menu2=="3"){
        ...
        bilang(j);
        QString jcair3 = ui->tableWidget_realisasi_add->item(0,4)->text();
...
void Form::data_update_sementara_add()
{
    ...
    QString jml_2 = ui->tableWidget_realisasi_add->item(0,4)->text(); // ===============
    QString jml_4 = li_data_add.at(11); // ==================
...
void Form::update_data_add(QString id, QString tahap, QString date, QString jml, QString terbilang, QString noSrt1, QString noSrt2)
{

    QSqlQuery query;
    begin();
    QString cmd = "UPDATE pmk_yhk.add_cair SET thp_cair = :thp_cair,  jml_cair_add = :j_cair, tgl = :tgl , no_srt1 = :no_srt1 , no_srt2= :no_srt2 , j_terbilang= :terbil WHERE id_real = :id";
    query.prepare(cmd);
    ...
    if(ok) { commit();
                ...
                eb_v->close();
                memuatData_twRealisasiadd(qbx_id_kam->currentText());
                }
}
...
void Form::dataPdf_add(int row)
{
    ...
    QString l= lspdf;
    QFile fOut(path+"dtpdf_add1.txt");
    if(fOut.exists()){fOut.remove();}
    if(!fOut.open(QFile::WriteOnly | QFile::Text))
        { qInfo()<<"tdk Bisa Buka file dtpdf_add1.txt ";}
